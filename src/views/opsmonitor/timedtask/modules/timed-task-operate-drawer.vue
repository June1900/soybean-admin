...
function parseJsonField(text: string, label: string): { ok: boolean; value: unknown } {
  ...
  try {
    ...
    window.$message?.error(`${label} ${$t('page.opsMonitor.timedTask.form.jsonError')}`);
...
async function handleSubmit() {
  if (!model.name) {
    window.$message?.warning($t('page.opsMonitor.timedTask.form.nameRequired'));
    return;
  }
  if (!model.spec) {
    window.$message?.warning($t('page.opsMonitor.timedTask.form.cronRequired'));
    return;
  }
  const p = parseJsonField(paramsText.value, $t('page.opsMonitor.timedTask.form.params'));
  if (!p.ok) return;
  const h = parseJsonField(headerText.value, $t('page.opsMonitor.timedTask.form.header'));
  if (!h.ok) return;
  ...
  window.$message?.success(
    model.ID ? $t('page.opsMonitor.timedTask.form.updateSuccess') : $t('page.opsMonitor.timedTask.form.createSuccess')
  );
  emit('submitted');
  closeDrawer();
}
  ...
  <NDrawer :show="visible" display-directive="show" :width="520" @mask-click="closeDrawer" @close="closeDrawer">
    ...
    >
      <NForm label-placement="top">
        <NFormItem :label="$t('page.opsMonitor.timedTask.form.name')" required>
          <NInput v-model:value="model.name" :placeholder="$t('page.opsMonitor.timedTask.form.name')" />
        </NFormItem>
        <NFormItem :label="$t('page.opsMonitor.timedTask.form.description')">
          <NInput v-model:value="model.description" type="textarea" :rows="2" />
        </NFormItem>
        <NFormItem :label="$t('page.opsMonitor.timedTask.form.spec')" required>
          <div class="flex w-full gap-8px">
            <NInput v-model:value="model.spec" placeholder="如 @daily 或 */30 * * * *" class="flex-1" />
            <NSelect
              :placeholder="$t('page.opsMonitor.timedTask.form.cronTemplate')"
              style="width: 170px"
              clearable
              :options="[
        ...
        <NFormItem :label="$t('page.opsMonitor.timedTask.form.withSeconds')">
          <NSwitch v-model:value="model.withSeconds" />
          <span class="ml-2 text-12px text-gray-400">{{ $t('page.opsMonitor.timedTask.form.withSecondsHint') }}</span>
        </NFormItem>
        <NFormItem :label="$t('page.opsMonitor.timedTask.form.executor')" required>
          <NRadioGroup v-model:value="model.executorType">
            <NRadio value="method">{{ $t('page.opsMonitor.timedTask.form.methodExecutor') }}</NRadio>
            <NRadio value="http">{{ $t('page.opsMonitor.timedTask.form.httpExecutor') }}</NRadio>
          </NRadioGroup>
        </NFormItem>

        <template v-if="model.executorType === 'method'">
          <NFormItem :label="$t('page.opsMonitor.timedTask.form.method')" required>
            <NSelect
              v-model:value="model.methodName"
              :placeholder="$t('page.opsMonitor.timedTask.form.method')"
              filterable
              :options="methodOptions.map(m => ({ label: `${m.name} — ${m.description}`, value: m.name }))"
            />
          </NFormItem>
          <NFormItem :label="$t('page.opsMonitor.timedTask.form.params')">
            <NInput v-model:value="paramsText" type="textarea" :rows="4" placeholder='{"days": 30}' />
          </NFormItem>
        </template>

        <template v-else>
          <NFormItem :label="$t('page.opsMonitor.timedTask.form.url')" required>
            <NInput v-model:value="model.httpUrl" placeholder="https://example.com/hook" />
          </NFormItem>
          <NFormItem :label="$t('page.opsMonitor.timedTask.form.httpMethod')">
            <NSelect
          ...
          <NFormItem :label="$t('page.opsMonitor.timedTask.form.header')">
            <NInput v-model:value="headerText" type="textarea" :rows="2" placeholder='{"Authorization":"Bearer xxx"}' />
          </NFormItem>
          <NFormItem :label="$t('page.opsMonitor.timedTask.form.body')">
            <NInput v-model:value="model.httpBody" type="textarea" :rows="3" />
          </NFormItem>
          <NFormItem :label="$t('page.opsMonitor.timedTask.form.allowPrivate')">
            <NSwitch v-model:value="model.httpAllowPrivate" />
            <span class="ml-2 text-12px text-gray-400">
              {{ $t('page.opsMonitor.timedTask.form.allowPrivateHint') }}
            </span>
          </NFormItem>
        </template>

        <NFormItem :label="$t('page.opsMonitor.timedTask.form.enabled')">
          <NSwitch v-model:value="model.enabled" />
        </NFormItem>
      </NForm>
      <template #footer>
        <NSpace justify="end">
          <NButton @click="closeDrawer">{{ $t('page.opsMonitor.timedTask.form.cancel') }}</NButton>
          <NButton type="primary" :loading="loading" @click="handleSubmit">
            {{ $t('page.opsMonitor.timedTask.form.submit') }}
          </NButton>
        </NSpace>
      </template>
    </NDrawerContent>
